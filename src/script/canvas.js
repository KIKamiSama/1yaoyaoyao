...
function setNum() {
    ...
    for (var i = 0; i < 60; i++) { if (i % 5 == 0) { numBack.lineWidth = 5; var xPoint = Math.sin(i * 6 * 2 * Math.PI / 360) * 195; var yPoint = -Math.cos(i * 6 * 2 * Math.PI / 360) * 195;
...
function drawSecPin() { ctxBack.save();
...
function drawMinPin() { ctxBack.save();
    ctxBack.translate(250, 250);
...
function drawHouPin() { ctxBack.save();
    ctxBack.translate(250, 250);
...
function setPoint() { ctxBack.beginPath();
    ctxBack.fillStyle = 'black';
    ctxBack.arc(250, 250, 3, 0, 2 * Math.PI);
    ctxBack.stroke(); }

function showBack() {
    for (var i = 0; i < 60; i++) { ctxBack.save();
        ctxBack.translate(250, 250);
...
function degToRad(degree) {
    ...
    if (degree == 0) { result = 270 * factor; } else { result = degree * factor; }
    return result;
}
